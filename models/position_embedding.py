...
class PositionEmbeddingCoordsSine(nn.Module):
    ...
    def get_fourier_embeddings(self, xyz, num_channels=None, input_range=None):
        ...
        if num_channels is None:
            num_channels = self.gauss_B.shape[1] * 2 # 128

        bsize, npoints = xyz.shape[0], xyz.shape[1] # bsize: 1
        assert num_channels > 0 and num_channels % 2 == 0
        d_in, max_d_out = self.gauss_B.shape[0], self.gauss_B.shape[1]
        d_out = num_channels // 2
        assert d_out <= max_d_out # 64, 64
        ...
        ncoords = xyz.shape[1]
        if self.normalize:
            xyz = shift_scale_points(xyz, src_range=input_range) # normalize to 0-1 for x, y, z individually

        xyz *= 2 * np.pi # [1, 4794, 3]
        ...
        final_embeds = torch.cat(final_embeds, dim=2).permute(0, 2, 1) # [1, 128, 4794]
        return final_embeds
